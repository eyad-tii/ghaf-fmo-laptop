...
An FMO device logs from six places at once, and a healthy boot already contains plenty of
errors that have always been there. Reading a fresh journal cold, you cannot tell those
from the one line your change actually broke. Diffing against a known-good snapshot
answers the question you actually have: what is different now?
...
Ported from ghaf .claude/skills/ghaf-logs/scripts/diff-logs.py. The normalisers and the
benign list are upstream's unchanged: the VMs are built by ghaf's modules, so the noise
they produce is ghaf's noise.
"""
